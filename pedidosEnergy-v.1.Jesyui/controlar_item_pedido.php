 <?php
    ...
    // echo "<br> testando a variavel num_pedido_new: $num_pedido_new";

    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        $id_pedido_teste = $_POST['id_pedido'];              
        $numero_pedido_incluir = $_POST['num_pedido'];
        $qtd_solicitada = $_POST['QtdSelecionada'];
        $cod_item_recebido_pelo_selector = $_POST['itens'];
     
            
        //  echo "<br> testando a variavel hidden id pedido: $id_pedido_teste";
    ...
    if (isset($_POST['salvar_modificar_item'])) {
        ...
        AND ip.num_seq_item = $seq_item_modificar;";
                 ...
                 <tr>
                     <td colspan="2">
                         ...
                         <?php if (isset($_POST['salvar_itens'])) { ?>
                            <?php if (empty($id_pedido_teste)) {
                              header('Location: index.php');
                              //tlvz usar ajax para retornar resposta e mostrar erro no modal
                            }else{
                                $query_inclusao_item = "INSERT INTO item_pedido
                                (num_pedido, num_seq_item, cod_item, qtd_solicitada, pre_unitario)
                                VALUES ($id_pedido_teste, (SELECT ip.num_seq_item + 1 AS increment_num_seq_item
